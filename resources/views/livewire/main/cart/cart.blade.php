    ...
    <div class="flex flex-row gap-10 place-content-center justify-center">
        <div class="w-full lg:basis-2/3">
            ...
            @if (is_array($cart) && count($cart))
                <div class="mb-6 bg-white shadow-sm rounded-md border">
                    
                    {{-- Section title --}}
                    <div class="bg-gray-50 px-8 py-4 rounded-t-md">
                        <div class="-ml-4 -mt-4 flex justify-between items-center flex-wrap sm:flex-nowrap">
                            <div class="ml-4 mt-4">
                                <h3 class="text-base leading-6 font-black tracking-widest text-gray-600">{{ __('messages.t_shopping_cart') }}</h3>
                                <p class="text-xs font-normal text-gray-400">{{ __('messages.t_review_your_cart_before_proceed_to_checkout') }}</p>
                            </div>
                    ...
                    <div class="px-8">
                        <div class="mt-12 lg:grid lg:grid-cols-12 lg:gap-x-12 lg:items-start xl:gap-x-16">
                                ...
                                <ul role="list" class="border-gray-200 divide-y divide-gray-200">
                                    ...
                                    @foreach ($cart as $key => $item)
                                        <li class="flex py-6 sm:py-10" wire:key="shopping-cart-item-id-{{ $key }}">
                                            ...
                                            <div class="ltr:ml-4 rtl:mr-4 flex-1 flex flex-col justify-between ltr:sm:ml-6 rtl:sm:mr-6">
                                                <div class="relative ltr:pr-9 rtl:pl-9 sm:grid sm:grid-cols-2 sm:gap-x-6 ltr:sm:pr-0 rtl:sm:pl-0">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="flex justify-between">
                                                            <h3 class="text-sm">
                                                                <a href="{{ url('service', $item['gig']['slug']) }}" target="_blank" class="font-medium text-gray-700 hover:text-indigo-500"> {{ $item['gig']['title'] }} </a>
                                                            </h3>
                                                        </div>
                                                                    ...
                                                                    @foreach ($item['upgrades'] as $index => $upgrade)
                                                                        <div class="relative flex items-start" wire:key="shopping-cart-upgrade-id-{{ $key }}-{{ $index }}">
                                                                            <div class="flex items-center h-5">
                                                                                <input id="shopping-cart-upgrade-id-{{ $upgrade['id'] }}" wire:model="cart.{{ $key }}.upgrades.{{ $index }}.checked" type="checkbox" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-200 border-2 rounded-sm">
                                                                            </div>
                                                                            <div class="ltr:ml-3 rtl:mr-3 text-sm mt-[-3px]">
                                                                                <label for="shopping-cart-upgrade-id-{{ $upgrade['id'] }}" class="font-medium text-gray-500 text-xs cursor-pointer">{{ $upgrade['title'] }}</label>
                                                    ...
                                                    <div class="mt-4 sm:mt-0 sm:pr-9 text-center">
                                                        ...
                                                        <select id="shopping-cart-quantity-id-{{ $key }}" wire:model="cart.{{ $key }}.quantity" class="max-w-full rounded-md border border-gray-300 py-1.5 text-base leading-5 font-medium text-gray-700 text-left shadow-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                        ...
                                                        <p class="mt-1 text-sm font-medium text-gray-900">@money($this->itemTotalPrice($item['id']), settings('currency')->code, true)</p>
                            ...
                            {{-- Cart summary --}}
                            <section aria-labelledby="summary-heading" class="my-16 bg-gray-50 rounded-lg px-4 py-6 sm:p-6 lg:p-8 lg:mt-0 lg:col-span-12">
                            <h2 id="summary-heading" class="text-lg font-medium text-gray-900">{{ __('messages.t_cart_summary') }}</h2>
                    
                            <dl class="mt-6 space-y-4">

                                {{-- Subtotal --}}
                                <div class="flex items-center justify-between">
                                    <dt class="text-sm text-gray-600">{{ __('messages.t_subtotal') }}</dt>
                                    <dd class="text-sm font-medium text-gray-900">@money($this->subtotal(), settings('currency')->code, true)</dd>
                                </div>
                                ...
                                @if (settings('commission')->enable_taxes)
                                    <div class="border-t border-gray-200 pt-4 flex items-center justify-between">
                                        ...
                                        <dd class="text-sm font-medium text-gray-900">@money($this->taxes(), settings('currency')->code, true)</dd>
                                    </div>
                                @endif

                                {{-- Total --}}
                                <div class="border-t border-gray-200 pt-4 flex items-center justify-between">
                                    <dt class="text-base font-medium text-gray-900">{{ __('messages.t_total') }}</dt>
                                    <dd class="text-base font-medium text-gray-900">@money($this->total() + $this->taxes(), settings('currency')->code, true)</dd>
                                </div>
                ...
                <div class="flex-grow flex flex-col justify-center max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="py-12">
                      <div class="text-center">
                        ...
                        <div class="mt-6">
                          <a href="{{ url('/') }}" class="text-base font-medium text-indigo-600 hover:text-indigo-500">{{ __('messages.t_continue_shopping') }}<span aria-hidden="true"> →</span></a>
                        </div>
                      </div>
                    </div>
                </div>
