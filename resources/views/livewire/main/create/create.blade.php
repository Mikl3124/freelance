<div class="w-full px-2 md:px-0 md:max-w-3xl mx-auto" x-data="window.KgRWYPfkiUNBpnW" x-cloak>
    ...
    @if ($isFinished)
        ...
        <div class="bg-white justify-center pb-4 pt-5 px-4 rounded-md shadow-sm sm:align-middle sm:max-w-sm sm:p-6 sm:w-full mx-auto">
            <div>
                ...
                <div class="mt-3 text-center sm:mt-5">
                    <h3 class="text-md leading-6 font-medium text-gray-900" id="modal-title">{{ __('messages.t_gig_created') }}</h3>
                    <div class="mt-2">
                        <p class="text-sm text-gray-500">{{ __('messages.t_gig_created_subtitle') }}</p>
                    </div>
                </div>
            </div>
            <div class="mt-5 sm:mt-6">
                <a href="{{ $isFinished }}" class="inline-flex justify-center items-center w-full rounded uppercase tracking-widest border border-transparent shadow-sm px-4 py-3 bg-indigo-500 font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 text-xs">
