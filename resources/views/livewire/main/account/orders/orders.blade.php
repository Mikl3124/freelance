                    ...
                    <div class="py-6 px-4 sm:p-6 lg:pb-8 h-[calc(100%-80px)]">
                        ...
                        <div class="w-full mb-6">
                            @forelse ($orders as $order)
                                ...
                                {{-- List of orders --}}
                                <div class="bg-white border border-gray-200 rounded-md mb-10" wire:key="orders-{{ $order->uid }}">
                                    <div class="bg-gray-50 border-b sm:rounded-t-md flex items-center p-4  sm:p-6 sm:grid sm:grid-cols-4 sm:gap-x-6">
                                        ...
                                        <dl class="flex-1 grid grid-cols-2 gap-x-6 text-sm sm:col-span-3 sm:grid-cols-2 lg:col-span-2">
                                            ...
                                            {{-- Date placed --}}
                                            <div>
                                                <dt class="font-medium text-gray-900">{{ __('messages.t_date_placed') }}</dt>
                                                <dd class="mt-2 text-gray-500 text-xs">{{ format_date($order->placed_at, 'F j, Y h:i A') }}</dd>
                                            </div>

                                            {{-- Total amount --}}
                                            <div>
                                                <dt class="font-medium text-gray-900">{{ __('messages.t_total_amount') }}</dt>
                                                <dd class="mt-1 font-medium text-gray-900">@money($order->total_value, settings('currency')->code, true)</dd>
                                            </div>
                                        ...
                                        @foreach ($order->items as $item)
                                            <li class="p-4 sm:p-6">
                                                <div class="flex items-center sm:items-start">
                                                    ...
                                                    {{-- Gig thumbnail --}}
                                                    <div class="flex-shrink-0 w-24 h-24 bg-gray-200 rounded-lg overflow-hidden hidden md:block">
                                                    ...
                                                    <div class="flex-1 ltr:md:ml-6 rtl:md:mr-6 text-sm">
                                                        <div class="md:flex md:items-center md:justify-between md:space-x-4 rtl:space-x-reverse xl:pb-4">
                                                            <div>
                                                                ...
                                                                {{-- Item title --}}
                                                                <h1 class="text-base font-bold text-gray-900 hover:text-indigo-600 mb-2">
                                                                    <a href="{{ url('service', $item->gig->slug) }}" target="_blank">{{ $item->gig->title }}</a>
                                                                </h1>

                                                                {{-- Item quick stats --}}
                                                                <div class="inline-block md:!flex text-gray-500 text-xs space-y-4 md:space-y-0 md:space-x-6 lg:space-x-12 rtl:space-x-reverse">
                                                            ...
                                                            <div class="mt-4 flex space-x-3 md:mt-0">
                                                                <button type="button" data-dropdown-toggle="orders-{{ $order->uid }}-item-{{ $item->id }}-actions-dropdown" data-dropdown-placement="left-start" class="inline-flex justify-center px-2 py-2 border border-gray-200 shadow-sm text-xs font-medium rounded-full text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-0">
                                                                ...
                                                                <div id="orders-{{ $order->uid }}-item-{{ $item->id }}-actions-dropdown" class="z-10 hidden bg-white divide-y divide-gray-100 rounded shadow w-48 dark:bg-gray-700">
                                                                    <ul class="text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefault">

                                                                        {{-- Contact seller --}}
                                                                        <li>
                                                                            <a href="{{ url('messages/new', $item->owner?->username) }}" target="_blank" class="flex items-center px-4 py-2 hover:bg-gray-100 rounded-t">
                                                                                <i class="mdi mdi-email-fast text-[20px] ltr:mr-3 rtl:ml-3 text-gray-500"></i>
                                                                                <span class="text-xs font-medium text-gray-700">{{ __('messages.t_contact_seller') }}</span>
                                                                            </a>
                                                                        </li>
                                                                        ...
                                                                        @if ($item->refund)
                                                                            ...
                                                                            <li>
                                                                                <a href="{{ url('account/refunds/details', $item->refund->uid) }}" target="_blank" class="flex items-center px-4 py-2 hover:bg-gray-100">
                                                                                    <i class="mdi mdi-cash-refund text-[20px] ltr:mr-3 rtl:ml-3 text-gray-500"></i>
                                                                                    <span class="text-xs font-medium text-gray-700">{{ __('messages.t_refund_details') }}</span>
                                                                                </a>
                                                                            </li>
                                                                        ...
                                                                        @else
                                                                            ...
                                                                            @if ($can_refund)
                                                                                <li>
                                                                                    <a href="{{ url('account/refunds/request', $item->uid) }}" target="_blank" class="flex items-center px-4 py-2 hover:bg-gray-100">
                                                                                        <i class="mdi mdi-cash-refund text-[20px] ltr:mr-3 rtl:ml-3 text-gray-500"></i>
                                                                                        <span class="text-xs font-medium text-gray-700">{{ __('messages.t_request_refund') }}</span>
                                                                                    </a>
                                                                                </li>
                                                                            @endif
                                                                        ...
                                                                        @if ($item->status === 'delivered')
                                                                            <li>
                                                                                <a href="{{ url('account/orders/files?orderId=' . $order->uid . '&itemId=' . $item->uid) }}"  class="flex items-center px-4 py-2 hover:bg-gray-100">
                                                                        ...
                                                                        @if ($item->status === 'pending' && $item->order->invoice->status === 'paid')
                                                                            <li>
                                                                                <button type="button" x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_to_cancel_service') }}') ? $wire.cancel('{{ $item->uid }}') : ''" wire:loading.attr="disabled" wire:target="cancel('{{ $item->uid }}')" class="flex items-center px-4 py-2 hover:bg-gray-100 w-full">
                                                                        ...
                                                                        @if (($item->status === 'pending' || $item->status === 'proceeded') && $item->order->invoice->status === 'paid')
                                                                            <li>
                                                                                <a href="{{ url('account/orders/requirements?order=' . $order->uid . '&item=' . $item->uid) }}"  class="flex items-center px-4 py-2 hover:bg-gray-100 rounded-b">
                                                ...
                                                <div class="mt-6 sm:flex sm:justify-between">
                                                    ...
                                                    @if ($item->gig->requirements()->exists() && !$item->is_requirements_sent && ($item->status === 'pending' || $item->status === 'proceeded') && $item->order->invoice->status === 'paid')
                                                    ...
                                                    {{-- Footer actions --}}
                                                    <div class="mt-6 border-t border-gray-200 pt-4 flex items-center space-x-4 divide-x divide-gray-200 rtl:divide-x-reverse text-sm font-medium sm:mt-0 sm:ltr:ml-4 sm:rtl:mr-4 sm:border-none sm:pt-0 rtl:space-x-reverse">

                                                        {{-- View gig --}}
                                                        <div class="flex-1 flex justify-center">
                                                            <a href="{{ url('service', $item->gig->slug) }}" target="_blank" class="text-indigo-600 whitespace-nowrap hover:text-indigo-500">{{ __('messages.t_view_gig') }}</a>
                                                        </div>

                                                        {{-- Send requirements --}}
                                                        @if (($item->status === 'pending' || $item->status === 'proceeded') && $item->order->invoice->status === 'paid')
                                                            <div class="flex-1 ltr:pl-4 rtl:pr-4 flex justify-center">
                                                                <a href="{{ url('account/orders/requirements?order=' . $order->uid . '&item=' . $item->uid) }}" class="text-indigo-600 whitespace-nowrap hover:text-indigo-500">{{ __('messages.t_send_requirements') }}</a>
                                                            </div>
                                                        @endif
