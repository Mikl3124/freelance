                    ...
                    <div class="py-6 px-4 sm:p-6 lg:pb-8">
                        ...
                        <div class="mb-14 lg:flex lg:items-center lg:justify-between">
                            <div class="flex-1 min-w-0">
                                <h2 class="text-base leading-6 font-bold text-gray-900">{{ __('messages.t_delivered_work') }}</h2>
                                <p class="mt-1 text-sm text-gray-500">{{ __('messages.t_download_received_files_from_seller') }}</p>
                            </div>
                            @if (!$item->is_finished && $item->status === 'delivered')
                                <div class="mt-5 flex lg:mt-0 lg:ltr:ml-4 lg:rtl:mr-4">
                                    ...
                                    @if (!$item->refund)
                                        <span class="block">
                                            <a href="{{ url('account/refunds/request', $item->uid) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-sm shadow-sm text-xs font-bold tracking-wide text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                ...
                                <section>
                                    <div class="rounded-lg bg-white overflow-hidden border border-gray-200">
                                        ...
                                        <div class="bg-gray-50 px-8 py-4 rounded-t-md">
                                            <div class="ltr:-ml-4 rtl:-mr-4 -mt-4 flex justify-between items-center flex-wrap sm:flex-nowrap">
                                                <div class="ltr:ml-4 rtl:mr-4 mt-4">
                                                    <h3 class="text-base leading-6 font-black tracking-widest text-gray-600">{{ __('messages.t_conversation_with_seller') }}</h3>
                                                    <p class="text-xs font-normal text-gray-400">{{ __('messages.t_communicate_with_seller_about_changes') }}</p>
                                                </div>
                                        ...
                                        <div>
                                                        ...
                                                        <li wire:key="seller-deliver-order-msg-id-{{ $message->id }}">
                                                            <div class="relative pb-8">
                                                                ...
                                                                <div class="relative flex items-start space-x-3 rtl:space-x-reverse">
                                                                    <div class="relative">
                                                                        <img class="h-10 w-10 rounded-md bg-gray-400 flex items-center justify-center ring-8 ring-white object-cover" src="{{ src($message->from->avatar) }}" alt="{{ $message->from->username }}">
                                                                    </div>
                                                                    <div class="min-w-0 flex-1">
                                                                    <div>
                                                                        <div class="text-sm">
                                                                            <a href="{{ url('profile', $message->from->username) }}" target="_blank" class="font-medium text-gray-900">{{ $message->from->username }}</a>
                                                                        </div>
                                            ...
                                            {{-- Send message --}}
                                            @if (!$item->is_finished)
                                                <div class="mt-auto w-full px-8 py-10 border-t bg-gray-50 rounded-br-md">
                                                        ...
                                                        <div class="min-w-0 flex-1">
                                                            <div class="relative">
                                                                <div class="border border-gray-300 rounded-lg shadow-sm overflow-hidden focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500">
                                                                    <textarea rows="3" maxlength="750" wire:model.defer="message" class="block w-full py-3 border-0 resize-none focus:ring-0 sm:text-sm" placeholder="{{ __('messages.t_type_ur_message_here') }}"></textarea>
                                    ...
                                    <section class="mb-6">
                                        <div class="rounded-lg bg-white overflow-hidden border border-gray-200">
                                            <div class="bg-gray-50 px-8 py-4 rounded-t-md">
                                                <div class="ltr:-ml-4 rtl:-mr-4 -mt-4 flex justify-between items-center flex-wrap sm:flex-nowrap">
                                                    <div class="ltr:ml-4 rtl:mr-4 mt-4">
                                                        <h3 class="text-base leading-6 font-black tracking-widest text-gray-600">{{ __('messages.t_download_files') }}</h3>
                                                        <p class="text-xs font-normal text-gray-400">{{ __('messages.t_always_scan_files_before_open_delivered_work') }}</p>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="px-8 py-5">
                                                ...
                                                <div class="flex items-center justify-between space-x-3 mb-6 rtl:space-x-reverse">
                                                    <div class="min-w-0 flex-1 flex items-center space-x-3 rtl:space-x-reverse">
                                                        ...
                                                        <div class="min-w-0 flex-1">
                                                            <p class="text-sm font-medium text-gray-900 truncate pb-2">
                                                ...
                                                {{-- Download button --}}
                                                <a href="{{ url('uploads/delivered/' . $item->order->uid . '/' . $item->uid . '/' . $item->delivered_work->id . '/' . $item->delivered_work->attached_work['id']) }}" target="_blank" class="w-full inline-flex items-center justify-center py-3 px-4 border border-transparent rounded-full bg-gray-100 hover:bg-gray-200 focus:outline-none focus:ring-0 mb-6">
                                                    ...
                                                    <div class="flex flex-col space-y-4 overflow-y-auto">
                                                        <div class="flex items-center">
                                                            <div class="flex flex-col space-y-2 text-sm ltr:ml-2 rtl:mr-2 order-2 items-center">
                                                                <div>
                                                                    <span class="px-4 py-2 rounded-lg italic inline-block bg-gray-100 text-gray-900 w-full">
