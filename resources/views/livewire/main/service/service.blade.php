<div class="relative -mx-4 md:mx-auto " x-data="window.oZcfXWmBuWfxbIo" x-init="initialize()">
        ...
        <div class="w-full mb-0 md:mb-12">
            ...
            <div class="md:flex items-center justify-between mb-0 md:mb-6">
                ...
                <div class="hidden items-center md:!grid">
                    ...
                    <span class="text-black !font-['Lato'] text-2xl tracking-wide font-black">@money($gig->price, settings('currency')->code, true)</span>
                </div>
        ...
        <div class="lg:grid lg:grid-rows-1 lg:grid-cols-7 lg:gap-x-8 lg:gap-y-10 xl:gap-x-16">
                        ...
                        <section id="main-carousel" class="splide rounded-xl">
                            <div class="splide__track">
                                    <ul class="splide__list">
                                        ...
                                        @if ($gig->video_link)
                                            <li class="splide__slide" data-splide-youtube="{{ $gig->video_link }}">
            ...
            <div class="max-w-2xl mx-auto mt-6 md:mt-14 sm:mt-16 lg:max-w-none lg:mt-0 lg:row-end-2 lg:row-span-2 lg:col-span-3">
                ...
                <div class="flex flex-col-reverse">
                    ...
                    <div class="flex items-center">
                        <div class="flex items-center" wire:ignore>
                            ...
                            @if ($gig->rating == 0)
                                <div class="text-[13px] font-black font-[Lato] text-gray-500 ltr:ml-2 rtl:mr-2">{{ __('messages.t_n_a') }}</div>
                            @else
                                <div class="text-[13px] font-black font-[Lato] text-amber-500 ltr:ml-2 rtl:mr-2">{{ $gig->rating }}</div>
                            @endif
                ...
                @if ($gig->upgrades()->count())
                    <div class="mt-6">
                        @foreach ($gig->upgrades as $key => $upgrade)
                            <label class="relative block py-4 cursor-pointer sm:flex sm:justify-between focus:outline-none bg-white" wire:key="add-to-cart-upgrades-{{ $key }}">
                                <div class="flex items-center">
                                    <input type="checkbox" value="{{ $upgrade->uid }}" wire:model.defer="upgrades" class="w-4 h-4 text-indigo-500 bg-transparent cursor-pointer rounded-sm ltr:mr-4 rtl:ml-4 border-2 border-gray-300 focus:ring-indigo-600">
                                    <div class="text-sm">
                                        <p id="server-size-0-label" class="font-medium text-gray-600 text-sm">{{ $upgrade->title }}</p>
                                ...
                                <div class="mt-2 flex text-sm sm:mt-0 ltr:sm:ml-4 rtl:sm:mr-4 sm:text-right items-center justify-center">
                                    <div class="font-bold text-xs text-black"><span class="pr-1 font-normal text-gray-500">+</span>@money($upgrade->price, settings('currency')->code, true)</div>
                                </div>
                ...
                <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">
                    ...
                    {{-- Contact seller --}}
                    <a href="{{ url('messages/new', $gig->owner->username) }}" target="_blank" class="w-full bg-indigo-50 border border-transparent rounded-md py-4 px-8 flex items-center justify-center text-[13px] font-medium text-indigo-700 hover:bg-indigo-100 focus:outline-none">{{ __('messages.t_contact_seller') }}</a>
                ...
                @if ($gig->documents()->count())
                    <div class="border-t border-gray-200 mt-10 pt-10">
                        <h3 class="text-sm font-medium text-gray-900">{{ __('messages.t_documents') }}</h3>
                ...
                <div class="border-t border-gray-200 mt-10 pt-10">
                    <h3 class="text-sm font-medium text-gray-900">{{ __('messages.t_actions') }}</h3>
                    <div class="mt-4 text-gray-500">
                        <ul role="list" class="flex items-center space-x-6 mt-4 rtl:space-x-reverse">
                            ...
                            <div class="flex items-center group cursor-pointer" id="modals-share-btn">
                                ...
                                <span class="text-sm font-medium text-gray-600 group-hover:text-gray-900 hidden md:block">{{ __('messages.t_share') }}</span>
                            </div>
                            ...
                            @endguest
                                ...
                                <span class="text-sm font-medium text-gray-600 group-hover:text-red-600 hidden md:block">{{ __('messages.t_report') }}</span>
                            </div>
                                ...
                                @if (auth()->id() !== $gig->user_id)
                                    ...
                                    @if ($inFavorite)
                                        <div class="flex items-center group cursor-pointer" wire:click="removeFromFavorite">
                                            ...
                                            <span class="text-sm font-medium text-indigo-600 hidden md:block">{{ __('messages.t_remove_from_favorite') }}</span>
                                        </div>
                                    @else
                                        <div class="flex items-center group cursor-pointer" wire:click="addToFavorite">
                                            ...
                                            <span class="text-sm font-medium text-gray-600 group-hover:text-indigo-600 hidden md:block">{{ __('messages.t_add_to_favorite') }}</span>
                                        </div>
                                    @endif
                                @endif
                            @endauth
                ...
                <div x-data="Components.tabs()" @tab-click.window="onTabClick" @tab-keydown.window="onTabKeydown">
                    ...
                    <div id="tab-panel-reviews" class="-mb-10" x-data="Components.tabPanel(0)" aria-labelledby="tab-reviews" x-init="init()" x-show="selected" @tab-select.window="onTabSelect" role="tabpanel" tabindex="0">
                        ...
                        <div class="py-6">
                            @foreach ($gig->tags as $tag)
                                <a href="{{ url('search?q=' . $tag->slug) }}" class="mb-3 text-xs font-semibold inline-block py-2 px-4 rounded text-slate-600 bg-gray-100 hover:bg-gray-200 last:mr-0 ltr:mr-1 rtl:ml-1">
                                    {{ $tag->name }}
                                </a>
                            @endforeach
                        </div>
                    ...
                    <div id="tab-panel-license" class="pt-10" x-data="Components.tabPanel(0)" aria-labelledby="tab-license" x-init="init()" x-show="selected" @tab-select.window="onTabSelect" role="tabpanel" tabindex="0" style="display: none;">
                                                    ...
                                                    <a href="{{ url('profile', $review->user->username) }}" target="_blank" class="text-sm font-bold text-gray-900 flex items-center group-hover:text-indigo-600">
                                                        ...
                                                        @if ($review->user->country)
                                                            <div class="ltr:ml-2 rtl:mr-2">
                                                                <img src="{{ url('public/img/flags/'. strtolower($review->user->country?->code) .'.svg') }}" alt="{{ $review->user->country?->name }}" class="h-3 -mt-px rounded-sm">
                                                            </div>
                                                        @endif
                        ...
                        @else
                            ...
                            <div class="text-center block text-sm font-normal text-gray-500">{{ __('messages.t_no_data_to_show_now') }}</div>
