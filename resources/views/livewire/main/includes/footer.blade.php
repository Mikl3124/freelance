<footer class="bg-white px-4 sm:px-6 lg:px-60 pt-20 pb-10 lg:pb-20 relative z-10" x-data="window.vZeNLeukZPSQcld">
    <div class="container mx-auto px-4">
                            ...
                            @if ($page->column == 1)
                                <li>
                                    @if ($page->is_link && $page->link)
                                        <a href="{{ $page->link }}" target="_blank" class="block font-normal text-gray-400 hover:text-gray-600 hover:underline text-sm mb-2">
                                            {{ $page->title }}
                                        </a>
                                    @else
                                        <a href="{{ url('page', $page->slug) }}" class="block font-normal text-gray-400 hover:text-gray-600 hover:underline text-sm mb-2">
                                            {{ $page->title }}
                                        </a>
                                    @endif
                                </li>
            ...
            <div class="col-span-3">
                ...
                @if (settings('footer')->is_language_switcher)
                    <button @click="languages = !languages" class="inline-flex items-center px-2.5 py-2 border border-gray-300 shadow-sm text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 mt-10">
                        ...
                        <span class="text-xs font-medium text-gray-500">{{ $default_language_name }}</span>
            ...
            <div class="flex items-center justify-center md:justify-items-start mb-4 md:mb-0">
                ...
                <div>
                    <a href="{{ url('/') }}" class="h-full">
                        <img src="{{ src(settings('footer')->logo) }}" alt="{{ settings('general')->title }}" class="py-2.5 max-h-10">
                    </a>
                </div>
        ...
        <div class="text-center mt-10">
            ...
            <div class="flex space-x-4 justify-center rtl:space-x-reverse">
                ...
                {{-- Contact --}}
                <a href="{{ url('help/contact') }}" class="text-xs font-medium uppercase text-gray-600 hover:underline rtl:ml-4">{{ __('messages.t_contact_us') }}</a>

                {{-- Privacy policy --}}
                @if (settings('footer')->privacy)
                    <a href="{{ url('page', settings('footer')->privacy->slug) }}" class="text-xs font-medium uppercase text-gray-600 hover:underline">{{ __('messages.t_privacy_policy') }}</a>
                @endif

                {{-- Terms --}}
                @if (settings('footer')->terms)
                    <a href="{{ url('page', settings('footer')->terms->slug) }}" class="text-xs font-medium uppercase text-gray-600 hover:underline rtl:!ml-0">{{ __('messages.t_terms_of_service') }}</a>
                @endif

                {{-- Blog --}}
                @if (settings('blog')->enable_blog)
                    <a href="{{ url('blog') }}" class="text-xs font-medium uppercase text-gray-600 hover:underline rtl:!ml-0">{{ __('messages.t_blog') }}</a>
                @endif
