<div class="w-full" x-data="window.AxZSIHcEeIYErvQ" x-init="init()" @keydown.window.escape="open = false" x-cloak>
    ...
    <div x-show="open" class="fixed inset-0 flex z-40 lg:hidden" x-ref="dialog" aria-modal="true">
        ...
        <div x-show="open" x-transition:enter="transition ease-in-out duration-300 transform" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in-out duration-300 transform" x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full" class="ml-auto relative max-w-xs w-full h-full bg-white shadow-xl py-4 pb-12 flex flex-col overflow-y-auto" style="display: none;">
            ...
            <div class="px-4 flex items-center justify-between">
                <h2 class="text-lg font-medium text-gray-900">{{ __('messages.t_filter') }}</h2>
                    ...
                    <div class="pt-6 px-4" x-show="open" style="display: none;">
                        <div class="space-y-4">
                            ...
                            @foreach ($delivery_times as $key => $time)
                                <div class="flex items-center">
                                    <input wire:model.defer="delivery_time" id="filter-delivery-time-{{ $key }}" value="{{ $time['value'] }}" name="delivery_time" type="radio" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300">
                                    <label for="filter-delivery-time-{{ $key }}" class="ltr:ml-3 rtl:mr-3 block text-sm font-medium text-gray-700">
                                        {{ $time['text'] }}
                                    </label>
                                </div>
                            @endforeach
    ...
    <main class="max-w-7xl mx-auto">
        ...
        <div class="flex justify-between items-center mb-2 bg-transparent py-2 ltr:pr-6 rtl:pl-6 ltr:border-l-8 rtl:border-r-8 ltr:pl-4 rtl:pr-4 border-indigo-600 rounded">
            ...
            {{-- Subcategory name --}}
            <div>
                <span class="font-extrabold text-base text-gray-800 pb-1 block tracking-wider">{{ $subcategory->name }}</span>
                    ...
                    <div x-data="Components.menu({ open: false })" x-init="init()" @keydown.escape.stop="open = false; focusButton()" @click.away="onClickAway($event)" class="relative inline-block ltr:text-left rtl:text-right">
                        ...
                        <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="ltr:origin-top-right rtl:origin-top-left absolute ltr:right-0 rtl:left-0 mt-2 w-40 rounded-md shadow-sm bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-50" x-ref="menu-items" x-bind:aria-activedescendant="activeDescendant" role="menu" aria-orientation="vertical" tabindex="-1" @keydown.arrow-up.prevent="onArrowUp()" @keydown.arrow-down.prevent="onArrowDown()" @keydown.tab="open = false" @keydown.enter.prevent="open = false; focusButton()" @keyup.space.prevent="open = false; focusButton()" style="display: none;">
                            <div class="py-1" role="none">
                                ...
                                {{-- Most popular --}}
                                <button wire:click="$set('sort_by', 'popular')" type="button" class="{{ $sort_by === 'popular' ? 'text-gray-900' : 'text-gray-500' }} block px-4 py-3 text-xs font-medium hover:bg-gray-100 w-full ltr:text-left rtl:text-right">
                                    {{ __('messages.t_most_popular') }}
                                </button>

                                {{-- Best rating --}}
                                <button wire:click="$set('sort_by', 'rating')" type="button" class="{{ $sort_by === 'rating' ? 'text-gray-900' : 'text-gray-500' }} block px-4 py-3 text-xs font-medium hover:bg-gray-100 w-full ltr:text-left rtl:text-right">
                                    {{ __('messages.t_best_rating') }}
                                </button>

                                {{-- Best selling --}}
                                <button wire:click="$set('sort_by', 'sales')" type="button" class="{{ $sort_by === 'sales' ? 'text-gray-900' : 'text-gray-500' }} block px-4 py-3 text-xs font-medium hover:bg-gray-100 w-full ltr:text-left rtl:text-right">
                                    {{ __('messages.t_most_selling') }}
                                </button>

                                {{-- Newest first --}}
                                <button wire:click="$set('sort_by', 'newest')" type="button" class="{{ $sort_by === 'newest' ? 'text-gray-900' : 'text-gray-500' }} block px-4 py-3 text-xs font-medium hover:bg-gray-100 w-full ltr:text-left rtl:text-right">
                                    {{ __('messages.t_newest_first') }}
                                </button>

                                {{-- Price: Low to High --}}
                                <button wire:click="$set('sort_by', 'price_low_high')" type="button" class="{{ $sort_by === 'price_low_high' ? 'text-gray-900' : 'text-gray-500' }} block px-4 py-3 text-xs font-medium hover:bg-gray-100 w-full ltr:text-left rtl:text-right">
                                    {{ __('messages.t_price_low_to_high') }}
                                </button>

                                {{-- Price: High to Low --}}
                                <button wire:click="$set('sort_by', 'price_high_low')" type="button" class="{{ $sort_by === 'price_high_low' ? 'text-gray-900' : 'text-gray-500' }} block px-4 py-3 text-xs font-medium hover:bg-gray-100 w-full ltr:text-left rtl:text-right">
                    ...
                    <div x-data="{ open: true }" class="py-3">
                        ...
                        <div class="pt-6 px-4" x-show="open" style="display: none;">
                            <div class="space-y-4">
                                ...
                                <div class="rounded-md shadow-sm -space-y-px">
                                    <div>
                                        <input type="integer" wire:model.defer="min_price" class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm font-medium" placeholder="{{ __('messages.t_min_price') }}">
                                    </div>
                                    <div>
                                        <input type="integer" wire:model.defer="max_price" required="" class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm font-medium" placeholder="{{ __('messages.t_max_price') }}">
                                    </div>
                                </div>
