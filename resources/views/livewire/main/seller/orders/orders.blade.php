                            ...
                            <table class="w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr class="divide-x divide-gray-50">
                                        ...
                                        <th scope="col" class="px-3 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.t_profit') }}</th>
                                        <th scope="col" class="px-3 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.t_expected_delivery_date') }}</th>
                                    ...
                                    @forelse ($orders as $order)                                        
                                        <tr wire:key="seller-orders-id-{{ $order->uid }}">
                                            ...
                                            <td class="whitespace-nowrap py-4 ltr:pl-10 rtl:pr-10 ltr:pr-3 rtl:pl-3 text-sm font-medium text-gray-900 sm:ltr:pl-10 sm:rtl:pr-10">
                                                <div class="flex items-center">
                                                    ...
                                                    <div class="ltr:ml-4 rtl:mr-4">
                                                        <a href="{{ url('service', $order->gig->slug) }}" target="_blank" class="font-medium text-gray-900 pb-1 hover:text-indigo-600 block">{{ $order->gig->title }}</a>
                                                        <div class="text-gray-500 font-normal">
                                                            <nav class="flex" aria-label="Breadcrumb">
                                                                <ol role="list" class="flex items-center space-x-4 rtl:space-x-reverse">
                                                                    ...
                                                                    <li>
                                                                        <div class="flex items-center">
                                                                            <a href="{{ url('categories', $order->gig->category->slug) }}" target="_blank" class="text-xs font-normal text-gray-500 hover:text-gray-700">{{ $order->gig->category->name }}</a>
                                                                        </div>
                                                                    </li>
                                                                    ...
                                                                    <li>
                                                                        <div class="flex items-center">
                                                                            ...
                                                                            <a href="{{ url('categories/' . $order->gig->category->slug . '/' . $order->gig->subcategory->slug) }}" target="_blank" class="ltr:ml-4 rtl:mr-4 text-xs font-normal text-gray-500 hover:text-gray-700" aria-current="page">{{ $order->gig->subcategory->name }}</a>
                                                                        </div>
                                                                    </li>
                                                                </ol>
                                                            </nav>
                                            ...
                                            <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 text-center">
                                                <div class="text-gray-900 text-sm font-bold font-[Lato]">@money($order->profit_value, settings('currency')->code, true)</div>
                                            </td>

                                            {{-- Expected delivery time --}}
                                            <td class="text-center">
                                                @if ($order->expected_delivery_date)
                                                    <span class="text-xs font-medium text-gray-700">{{ format_date($order->expected_delivery_date, 'F j, Y h:i A') }}</span>
                                            ...
                                            <td class="whitespace-nowrap text-center py-4 px-3">
                                                ...
                                                @elseif ($order->order->invoice->status === 'pending')
                                            ...
                                            <td class="text-center">
                                                <div class="relative inline-block text-left">
                                                    <div>
                                                        <button data-dropdown-toggle="seller-orders-options-{{ $order->uid }}" data-dropdown-placement="left-start" type="button" class="inline-flex justify-center items-center rounded-full h-8 w-8 bg-white hover:bg-gray-50 focus:outline-none focus:ring-0" aria-expanded="true" aria-haspopup="true">
                                                    ...
                                                    <div id="seller-orders-options-{{ $order->uid }}" class="hidden z-40 origin-top-right absolute right-0 mt-2 w-44 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">

                                                        <div class="py-1" role="none">

                                                            {{-- View refund --}}
                                                            @if ($order->refund)
                                                                <a href="{{ url('seller/refunds/details', $order->refund->uid) }}" class="text-gray-800 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0">
                                                            ...
                                                            {{-- Get started now --}}
                                                            @if ($order->status === 'pending' && $order->order->invoice->status !== 'pending')
                                                                <div class="py-1" role="none">
                                                                    <button @if (!$order->expected_delivery_date) x-on:click="progress('{{ $order->uid }}')" @else wire:click="progress('{{ $order->uid }}')" @endif wire:loading.attr="disabled" wire:target="progress('{{ $order->uid }}')" type="button" class="text-gray-800 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" >
                                                            ...
                                                            {{-- Details --}}
                                                            <a href="{{ url('seller/orders/details', $order->uid) }}" class="text-gray-800 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0">
                                                                <svg xmlns="http://www.w3.org/2000/svg" class="ltr:mr-3 rtl:ml-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"> <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                                                <span class="text-xs font-medium">{{ __('messages.t_order_details') }}</span>
                                                            </a>

                                                            {{-- Contact buyer --}}
                                                            <a href="{{ url('messages/new', $order->order->buyer->username) }}" target="_blank" class="text-gray-800 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0">
                                                            ...
                                                            @if (in_array($order->status, ['proceeded', 'delivered']) && !$order->is_finished)
                                                                <a href="{{ url('seller/orders/deliver', $order->uid) }}" class="text-gray-800 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0">
                                                                    <svg xmlns="http://www.w3.org/2000/svg" class="ltr:mr-3 rtl:ml-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"> <path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
                                                            ...
                                                            @if ($order->requirements()->count())
                                                                <a href="{{ url('seller/orders/requirements', $order->uid) }}" class="text-gray-800 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0">
                                                        ...
                                                        @if ($order->status === 'pending' && $order->order->invoice->status !== 'pending')
                                                            <div class="py-1" role="none">
                                                                <button x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_to_cancel_order') }}') ? $wire.cancel('{{ $order->uid }}') : ''" wire:loading.attr="disabled" wire:target="cancel('{{ $order->uid }}')" type="button" class="text-gray-800 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" >
